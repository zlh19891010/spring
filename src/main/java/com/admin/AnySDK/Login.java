...
public class Login {
	...
	public boolean check( HttpServletRequest request, HttpServletResponse response ) {
		...
		try{
			...
			sendToClient( response, CatchRes(result) );
			return true;
		} catch( Exception e ) {
			e.printStackTrace();
		}
	 ...
	 * @throws IOException
	 */
	private static String CatchRes(String result) throws  IOException{

		SessionMap map=  SessionMap.newInstance();
		JSONObject json=(JSONObject) JSONObject.parse(result);

		//		JSONObject json=(JSONObject) JSON.parse(result);
		map.addCache(json.getJSONObject("data").getString("openid"), json.getJSONObject("data").getString("user_info"));
		JSONObject ext =new JSONObject();
		ext.put("openid",json.getJSONObject("data").getString("openid"));
		json.put("ext", ext);
	...
	/*public static void main(String[] args) {
		String str="{\"data\": {\"openid\": \"oVYdf0sgD8NdDZs0Kfxt5omuW6Y4\"},\"ext\": \"\"}";
		try {
			System.err.println(CatchRes(str));
